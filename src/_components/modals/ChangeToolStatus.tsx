...
export default function ChangeToolStatus({ open, onClose, name }: Props) {
    const [action, setAction] = useState<boolean | null>(false)
                ...
                <div className="flex flex-col items-left">
                    <div className="mt-3 text-center sm:text-left">
                        ...
                        <p className="text-xxs sm:text-xs md:text-xs 2xl:text-sm text-gray-500 mb-4">
                            Please select the status and condition of this tool below and leave a comment. Also inidicate if it requires action.
                        </p>
                        <form action="" className='mt-2 flex flex-col gap-y-3'>
                            <div className='flex flex-col gap-y-0.5'>
                                <label htmlFor="" className='text-xs font-medium text-gray-600'>New Status</label>
                            ...
                            <div className='flex flex-col gap-y-0.5'>
                                <label htmlFor="" className='text-xs font-medium text-gray-600'>Comments</label>
                            ...
                            <div className='flex flex-col gap-y-0.5'>
                                <label htmlFor="" className='text-xs font-medium text-gray-600'>Requires Action?</label>
                                <select 
                                    name="action-required" 
                                    id="action-required" 
                                    className='border border-gray-200 w-full rounded-sm px-2 py-1.5 text-xxs md:text-xs outline-none focus:border-green-600' 
                                    // Convert the boolean state to a string for the select value
                                    value={action ? "true" : "false"} 
                                    // Check if the incoming string is equal to "true" to set the boolean
                            ...
                            
                            {
                                action &&
                                <div className='flex flex-col gap-y-0.5'>
                                    <label htmlFor="" className='text-xs font-medium text-gray-600'>What Action is needed?</label>
                                    <select name="" id="" className='border border-gray-200 w-full rounded-sm  px-2 py-1.5 text-xxs md:text-xs outline-none focus:border-green-600'>
                                        <option value="">Requires Mainenance</option>
                                    </select>
                                </div>
                            }
                        </form>
